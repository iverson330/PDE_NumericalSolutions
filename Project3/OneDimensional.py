...
class Solver:
    ...
    def solve_engine(self):
        ...
        fig, ax = plt.subplots()
        computed_line, = ax.plot(self.x_list[1:self.N+1], self.U_vector, 'bx')
        real_line, = ax.plot(self.x_list[0:self.N+1], self.realU_vector)

        error_list = (self.realU_vector[1: self.N+1] - self.U_vector) / self.realU_vector[1: self.N+1]
        print("Max error = ", np.max(error_list))

        ax.set_xlabel("x")
        ax.set_ylabel("u(x)")
        ax.set_title("Method = %s, N = %s." % (str(self.method), str(self.N)))
        ax.legend((real_line, computed_line), ('Real solution', 'Computed'), loc='upper left')
        #
